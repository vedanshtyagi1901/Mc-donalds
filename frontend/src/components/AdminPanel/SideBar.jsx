...
import Customer_footsteps from './Customer-Relationship/Customer_footsteps';

function SideBar() {
    const [content, setContent] = useState('');
    const [analyticsOpen, setAnalyticsOpen] = useState(false); // State to manage analytics dropdown
    const [CustomerManagementOpen, setCustomerManagementOpen] = useState(false); // State to manage analytics dropdown

    return (
        <>
            <div className='flex bg-[rgb(16,20,34)]'>
                <div className="h-screen w-80 bg-gray-800 text-white p-4">
                    {/* Logo Section */}
                    ...
                    <nav>
                        <ul>
                            ...
                            <li>
                                <a
                                    href="#"
                                    className="flex items-center justify-between py-3 px-4 rounded-lg hover:bg-indigo-600 transition duration-300"
                                ...
                                >
                                    ...
                                    <svg
                                        ...
                                        className={`w-5 h-5 transform transition-transform ${analyticsOpen ? 'rotate-180' : ''}`}
                                        fill="none"
                                        viewBox="0 0 24 24"
                                        stroke="currentColor"
                                    >
                            ...
                            <li>
                                <a
                                    href="#"
                                    className="flex items-center justify-between py-3 px-4 rounded-lg hover:bg-indigo-600 transition duration-300"
                                    onClick={() => setCustomerManagementOpen(!CustomerManagementOpen)} // Toggle dropdown
                                >
                                    <div className="flex items-center space-x-4">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m4-4h-4m-4 8h4m6 0h-2a2 2 0 01-2-2V7a2 2 0 012-2h2M5 3v18a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2z" />
                                        </svg>
                                        <span className="text-lg">Customer Management</span>
                                    </div>
                                    {/* Icon to indicate open/close */}
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        className={`w-5 h-5 transform transition-transform ${CustomerManagementOpen ? 'rotate-180' : ''}`}
                                        fill="none"
                                        viewBox="0 0 24 24"
                                        stroke="currentColor"
                                    >
                                ...
                                {CustomerManagementOpen && (
                                    <ul className="pl-8 mt-2 space-y-2">
                                        <li onClick={() => setContent('Customer-Footsteps')}>
                                            <a href="#" className="flex items-center space-x-4 py-2 px-4 rounded-lg hover:bg-indigo-600 transition duration-300">
                                                <span className="text-lg">Customer Footsteps</span>
                                            </a>
                                        </li>
                                        <li onClick={() => setContent('Annual-Profit')}>
                                            <a href="#" className="flex items-center space-x-4 py-2 px-4 rounded-lg hover:bg-indigo-600 transition duration-300">
                ...
                {content === 'Customer-Footsteps' && <Customer_footsteps/>}
            </div>
        </>
    );
}
