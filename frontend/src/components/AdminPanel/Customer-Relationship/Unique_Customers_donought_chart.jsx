...
function Unique_Customers_Doughnut_Chart() {
  ...
  const fetchReservations = async (filterParams) => {
    try {
      ...
      // Extract unique customers using phone numbers
      const uniqueCustomers = new Set(filteredReservations.map((reservation) => reservation.phone));
  ...
  // Doughnut chart data
  const data = {
    labels: ['Unique Customers', ""],
    datasets: [
      {
        label: 'Unique Customers',
        data: [uniqueCustomerCount, 0], // Replace 100 with the actual total if needed
  ...
  return (
    <div className="p-2 bg-gray-800 text-white rounded-lg shadow-md w-[500px]">
      <h2 className="text-xl font-bold mb-2 text-center">Unique Customers</h2>
      ...
      <p className="mt-2 text-center text-gray-300">
        Total Unique Customers: <span className="font-bold text-white">{uniqueCustomerCount}</span>
      </p>
    </div>
  );
}

export default Unique_Customers_Doughnut_Chart;
