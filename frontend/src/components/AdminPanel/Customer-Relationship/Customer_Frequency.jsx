...
function Customer_Frequency() {
  const [visitData, setVisitData] = useState([]); // Initialize as an empty array
  const [selectedPeriod, setSelectedPeriod] = useState('daily');

  // Helper function to calculate visits based on the frequency
  const processVisitData = (reservations, period) => {
    ...
    reservations.forEach((reservation) => {
      ...
      // Count the unique visits for each period
      if (!visits[periodKey]) {
        visits[periodKey] = new Set();
      }
      visits[periodKey].add(phone);
    });

    // Calculate visit counts for each period and return it as an array
    return Object.keys(visits).map((key) => ({
      period: key,
      count: visits[key].size, // Unique phone numbers (customers) count
    }));
  };
    ...
    datasets: [
      {
        ...
        data: visitData.map((data) => data.count), // Y-axis data for unique customer count
        fill: false,
  ...
  return (
    <div className="p-2 bg-gray-800 text-white rounded-lg shadow-md mt-4 w-[500px] h-10">
      <h2 className="text-xl font-bold mb-4 text-center">Customer Frequency</h2>
...
export default Customer_Frequency;
